...
public class DayPageBean implements Serializable{
	 ...
	 */
		...
		DayDAO.getInstance().addDay(this.newday.getName(), this.newday.getCostperday(),this.newday.getCountry(),this.newday.getCurrency());
		UserAuditDAO.getInstance().addUserAudit("Add Rate", "User added the rate : "+this.newday.getName());
		newday=new Day();
		return "DaysPage?faces-redirect=true";
	}
	/**
	...
	public String modifyDay(){
		...
		if(this.selectedday!= null){
			DayDAO.getInstance().modifyDay(this.selectedday.getId(), this.selectedday.getName(), this.selectedday.getCostperday(),this.selectedday.getCountry(),this.selectedday.getCurrency());	
			UserAuditDAO.getInstance().addUserAudit("Modify Rate", "User modified the rate : "+this.selectedday.getName());
		}
		return "DaysPage?faces-redirect=true";
	}
	...
	public String deleteDay(){
		...
		if(this.selectedday!= null){
			...
			UserAuditDAO.getInstance().addUserAudit("Delete Rate", "User deleted the rate : "+this.selectedday.getName());
		}
		return "DaysPage?faces-redirect=true";
	}
}
